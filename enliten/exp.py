...
class LCOECalculator:
    ...
    def calculate_lcoe_metrics(self, system_capex_USD = None, system_annual_OM_USD = None, system_annual_VOM_USD = None, system_to_load_annual_MWh_e = None, system_augment = None, ITC = None, DF = None, COE = None, I = None, grant_percentage = None, tax  = None, inflation = None, property_tax = None, insurance = None, depreciation_period = None, esc = None, analysis_period = None, VOM = None, e_sale = None, gas_cost = None):
        ...
        print(f'WACC_n: {WACC_n} ')
        WACC_r = ((1 + WACC_n) / (1 + inflation)) - 1

        print(f'WACC_r: {WACC_r} ')
        ...
        print(f'NPV_system_augment_N {NPV_system_augment_N}')

        print(f'grant_percentage {grant_percentage}')
        print(f'system_capex_USD {system_capex_USD}')
        print(f'FCR_after_tax_deduction {FCR_after_tax_deduction}')
        ...
        print(f'annualized_system_augment_AT {annualized_system_augment_AT}')

        print(f'annualized_OM_AT {annualized_OM_AT}')
        print(f'annualized_VOM_AT {annualized_VOM_AT}')
        print(f'annualized_electricity_purchases_AT {annualized_electricity_purchases_AT}')
        print(f'annualized_electricity_sales_AT {annualized_electricity_sales_AT}')
        print(f'annualized_CAPEX_AT {annualized_CAPEX_AT}')
        ...
        annual_ARR_AT = (annualized_CAPEX_AT + annualized_OM_AT + annualized_VOM_AT + annualized_electricity_purchases_AT + annualized_system_augment_AT + 412619.34934613004)

        print(f'annual_ARR_AT {annual_ARR_AT}')

        annual_ARR_BT = (annualized_CAPEX_BT + annualized_OM_BT + annualized_VOM_BT + annualized_electricity_purchases_BT + annualized_system_augment_BT + 555342.3275183446)

        print(f'annual_ARR_AT {annual_ARR_AT}')


        annual_fuel = [0, 410328.256, 421817.447168, 433628.3356887,
 445769.92908799, 458251.48710245, 471082.52874132, 484272.83954608,
 497832.47905337, 511771.78846686, 526101.39854393, 540832.23770316,
 555975.54035885, 571542.8554889,  587546.05544259, 603997.34499498,
 620909.27065484, 638294.73023318, 656166.98267971, 674539.65819474,
 693426.76862419, 712842.71814567, 732802.31425375, 753320.77905285,
 774413.76086633, 796097.34617059, 818388.07186337, 841302.93787554,
 864859.42013605, 889075.48389986, 913969.59744906]
        ...
        for v in range(1, analysis_period + 1):
            ...
            ebit = (annual_electricity_sales[v]) - (
                OM_esc + annual_electricity_purchases[v] + system_augment[v] + annual_fuel[v]
            )
            ...
            costs.append(OM_esc + annual_electricity_purchases[v]+ system_augment[v] + annual_fuel[v]
            )
        ...
        print(f'after_tax_TLCC: {after_tax_TLCC}')
